        ...
        for ind, i in enumerate(W):
            ...
            for jnd, j in enumerate(i):
                if j==0:
                    ...
                    my_w[ind].append(jnd+1)
        return my_w

    @staticmethod
    def set_w_by_pre_w(W):
        my_w = list()
        for ind, i in enumerate(W):
            my_w.append(list())
            for jnd, j in enumerate(i):
                if j == 0:
                    #my_w[ind][jnd]=float('inf')
                    my_w[ind].append(float('inf'))
                else:
                    my_w[ind].append(W[ind][jnd])
        return my_w

    def floyd(self):
        print('Это адж матрица класса: ')
        adj_for_h = self.adj_mat
        pprint(self.adj_mat)
        H = Graph.set_h_by_pre_w(self.adj_mat)  # задаем матрицу истории
        print('Это матрица истории: ')
        pprint(H)
        print('Это адж матрица класса: ')
        pprint(self.adj_mat)
        W = Graph.set_w_by_pre_w(self.adj_mat)
        pprint(W)
        for a in self.nodes:
            for b in self.nodes:
                if W[a.index][b.index]!=float('inf'):
                    for c in self.nodes:
                        if W[a.index][c.index] > W[a.index][b.index] + W[b.index][c.index]:
                                W[a.index][c.index] = W[a.index][b.index] + W[b.index][c.index]
                                H[a.index][c.index] = H[a.index][b.index]
        return W, H
...
def get_dijkstara(graph, node):
    if isinstance(graph, Graph):
        res = [(weight, [n.data for n in node]) for (weight, node) in graph.dijkstra(node)]
        return res
    else:
        raise ValueError(f'Type of argument was not {Graph}. Type of {graph} = {type(graph)}')
...
def get_new_graph(list_of_nodes):
    return Graph.create_from_nodes(list_of_nodes)


def set_new_connection(my_graph, first_node, second_node, weight):
    if isinstance(my_graph, Graph):
        my_graph.connect(first_node, second_node, weight)
        return True
    else:
        raise ValueError
...
def main():
    # second_graph.connect(a, d, 2)
    # second_graph.print_adj_mat()

    # res = [(weight, [n.data for n in node]) for (weight, node) in second_graph.dijkstra(a)]
    a = get_new_node('A')
    b = get_new_node('B')
    c = get_new_node('C')
    d = get_new_node('D')
    ...
    my_graph = get_new_graph(list_of_nodes)

    set_new_connection(my_graph, a, b, 5)
    set_new_connection(my_graph, a, c, 2)
    set_new_connection(my_graph, b, c, 2)
    set_new_connection(my_graph, c, d, 6)
    set_new_connection(my_graph, b, e, 7)

    # res = my_graph.dijkstra(a)
    W, H = my_graph.floyd()
    print('final W is: ')
    pprint(W)
    print('final H is: ')
